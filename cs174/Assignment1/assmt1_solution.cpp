...
void drawPlant(float x,float y,float z)
{
	set_colour(0,0.5,0) ;
	glPushMatrix() ;
	glTranslatef(x,y,z) ;
	...
	for( int i = 0 ; i < 10 ; i++)
	{
		...
		glPopMatrix() ;
		glTranslatef(0,1,0) ;
		glRotatef( 20*cos(TIME+i*30),0,0,1) ;
		//glRotatef( r*45*cos(TIME),0,0,1) ;
		r = -r ;
	}
...
void drawFish(float x,float y,float z, float theta)
{
	

	glPushMatrix() ; // entire fish
	glTranslatef(x,y,z) ;
...
void display(void)
{
	...
	glPushMatrix();
	glTranslatef(0,-2,0) ; // entire scene
	glScalef(0.5,0.5,0.5) ;
	
	set_colour(0.3,0.4,0.2) ;
	glPushMatrix() ; // floor
	glScalef(60,1,60) ;
	...
	y = 1.5*cos(TIME)+3 ;
	//y = 3 ;
	float theta = -180*atan2(z,x) / PI ;
	drawFish(x,y,z,theta) ;
...
void idleCB(void)
{
	if( Animate == 1 )
	{
		...
		if( Recording == 0 )
			...
			TIME = TIME + 0.033 ; // save at 30 frames per second.
		//printf("TIME %f\n", TIME) ;
		glutPostRedisplay() ; 
	}
}
